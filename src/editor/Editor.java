...
public class Editor {
        ...
        try {
            ...
            if (lastAppSize != null) {
                ...
                if (splitSize.length == 2)
                    Application.SetWindowSize((int) Float.parseFloat(splitSize[0]), (int) Float.parseFloat(splitSize[1]));
            }
    ...
    public static void Render() {
        ...
        if (playing == 0) {
            ...
            GUI.Window(new Rect(0, 30, editorDrawArea.x, Application.Height() - 60), "Hierarchy", h::Render, window);
            ...
            if (Input.GetKey(KeyCode.LeftControl) && Input.GetKeyDown(KeyCode.S)) {
                try {
                    ...
                    Debug.Log("Scene could not be saved!");
                    e.printStackTrace();
                }
            }
        }
    ...
    public static final Rect GetSceneDrawArea() {
        return sceneDrawArea;
    }
    ...
    public static void DrawCollider(Camera camera, GameObject go) {
        ...
        if (go == null) return;
        SpriteRenderer sr = go.GetRenderer();
            ...
            if (corners != null) {
                if (col != null) {
                    if (col instanceof BoundsCollider) {
                        DrawRect(camera, Rect.GetBounds(corners), Color.green);
                    }
                }
            }
        }
        if (col != null) {
            if (col instanceof CircleCollider) {
                ...
                DrawCircle(camera, col.gameObject().Position().Add(cc.anchor), cc.radius, Color.green);
                ...
                if (corners != null) {
                    if (col != null) {
                        if (col instanceof BoundsCollider) {
                            DrawRect(camera, Rect.GetBounds(corners), Color.green);
                            continue;
                        } else DrawRect(camera, Rect.GetBounds(corners), Color.white);
                    } else DrawRect(camera, Rect.GetBounds(corners), Color.white);
                }
            }
            if (col != null) {
    ...
    private static void DrawRect(Camera c, Rect r, Color color) {
        ...
        GL11.glVertex2f(pos.x - r.width, pos.y - r.height);
        GL11.glVertex2f(pos.x - r.width, pos.y);
        GL11.glEnd();
    }
    ...
    private static void DrawCircle(Camera c, Vector2 p, float r, Color color) {
        ...
        for (int o = 0; o < 16; o++) {
            ...
            GL11.glVertex2d(pos.x + (r * Math.cos(angle)), pos.y + (r * Math.sin(angle)));
        }
        GL11.glEnd();
    }
