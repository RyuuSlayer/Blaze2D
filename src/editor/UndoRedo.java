...
public class UndoRedo {
	private static final List<UndoElement> undo = new ArrayList<UndoElement>();
	private static final List<UndoElement> redo = new ArrayList<UndoElement>();

	public static void RegisterUndo(Object obj, Field field) {
		Class<?> c = null;
		try {
			c = field.get(obj).getClass();
		...
		try {
			...
			else if (c == String.class) element.orginalValue = (String) field.get(obj);
			else if (c == Vector2.class) {
				element.orginalValue = ((Vector2) field.get(obj)).clone();
	...
	public static void Undo() {
		...
		Class<?> c = null;
		try {
			c = element.f.get(element.o).getClass();
		} catch (IllegalArgumentException | IllegalAccessException e) {
			e.printStackTrace();
		}
		...
		try {
			...
			else if (c == String.class) element.f.set(element.o, element.orginalValue);
			else if (c == Vector2.class) element.f.set(element.o, element.orginalValue);
			else if (c == Rect.class) element.f.set(element.o, element.orginalValue);
			else if (c.isInstance(engine.Object.class))
				element.f.set(element.o, element.f.get(element.o).getClass().cast(element.orginalValue));
		} catch (IllegalArgumentException | IllegalAccessException e) {
			e.printStackTrace();
		}
	}
